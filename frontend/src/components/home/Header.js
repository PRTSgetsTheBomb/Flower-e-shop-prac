 ...
 *
 * 设计说明�?
 * - 搜索�?navigate 跳转�?/search?q=xxx，将搜索参数放在 URL 中，保证结果页可分享、可刷新、可回退
 * - 使用 encodeURIComponent 防止用户输入 &�? 等特殊字符破�?URL 结构
 * - 滚动隐藏�?useRef 存上次滚动位置而非 useState，因为该值只需比较不需要触发重渲染
 * - 80px 阈值是经验值，作为"缓冲�?避免用户轻微滚动就触发隐�?
 * - useEffect 注册 scroll 监听并返回清理函数，组件卸载时移除监听防止内存泄�?
 * - { passive: true } 告诉浏览器不调用 preventDefault()，提升滚动性能
 * - �?sticky 定位而非 fixed，隐藏时页面内容自然上移不留下空�?
 * - NavLink 自动根据当前路由添加 active 类，无需手动判断
 * - Occasions 按钮手动�?some() + startsWith 判断，因为需要检测多个子路由中的任意一�?
 * - onBlur 关闭下拉菜单�?点击外部关闭"的事件监听更简�?
 * - AnimatePresence 能正确处理组件卸载时的退出动画，�?CSS 难以实现
 */
...
function Header() {
    ...
    const lastScroll = useRef(0); // �?ref 存滚动位置，避免 setState 触发不必要的重渲�?
    ...
    // �?Enter 时跳转到搜索页，搜索参数放在 URL 中方便分享和回退

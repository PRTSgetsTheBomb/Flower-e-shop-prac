 ...
 *
 * 1. 【条件渲染：空购物车 vs 有商品�?
 *    �?if (cart.length === 0) 提前 return 空状�?UI�?
 *    避免在有商品的情况下还要�?是否为空"的冗余判断�?
 *    两种状态的 UI 差异很大，分开写比一个大三元表达式更清晰�?
 *
 * 2. 【使�?useCart() 消费 Context�?
 *    useCart() �?CartContext 中解构出 cart 数据和操作方法�?
 *    这里不需要关心数据从哪来、怎么存，只管"�?就行�?
 *    这就�?Context �?数据管理"�?数据消费"解耦的好处�?
 *
 * 3. 【数量加减：disabled 防越界�?
 *    disabled={item.qty <= 1}：数量为 1 时禁用减号按钮�?
 *    因为 UPDATE_QTY �?Math.max(1, qty) 保护，即使按钮没被禁用，
 *    数量也不会减�?0 以下。但禁用按钮提供了更好的用户体验（视觉反馈）�?
 *
 * 4. 【价格格式化：toFixed(2)�?
 *    所有价格用 .toFixed(2) 确保显示两位小数，避免显�?$19.9 �?$19.999�?
 *    parseFloat 是因�?API 返回的价格可能是字符串类型�?
 *
 * 5. 【商品名称为 Link�?
 *    点击商品名称可跳转到商品详情�?/product/:slug�?
 *    方便用户在结账前查看商品详情�?
 *
 * 6. 【行小计 vs 总计�?
 *    每个商品行单独计算小计（单价 × 数量），
 *    底部总计使用 CartContext 中计算好�?totalPrice�?

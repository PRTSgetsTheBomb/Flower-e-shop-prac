 ...
 *
 * 核心职责：根�?URL 中的分类标识展示对应的商品列�?
 *
 * 设计说明�?
 *
 * 1. 【useParams 获取分类标识�?
 *    :slug �?URL 中获取，�?/collections/dried-flowers �?slug �?"dried-flowers"�?
 *    这个值决定调用哪�?API（全部商�?vs 按分类查询）和页面标题�?
 *
 * 2. 【有条件的数据获取�?
 *    如果 slug === 'available-today'，调�?fetchAllProducts(100) 获取所有商品�?
 *    否则调用 fetchProductsByCategory(slug) 获取特定分类的商品�?
 *    这种根据 URL 改变请求逻辑的模式在电商网站中很常见�?
 *
 * 3. 【useEffect 依赖 [slug]�?
 *    当用户在分类间切换时（如�?/collections/fresh-flowers �?dried-flowers），
 *    slug 变化触发 useEffect 重新执行，自动请求新分类的数据�?
 *    如果依赖数组�?[]，则只在组件挂载时请求一次，切换分类不会刷新数据�?
 *
 * 4. 【Promise.resolve 的作用�?
 *    fetchAllProducts �?fetchProductsByCategory 都是 async 函数（返�?Promise），
 *    �?Promise.resolve(fetch).then() 确保无论 fetch �?Promise 还是普通值，
 *    都能统一�?.then() 处理。这里主要为了类型一致性�?
 *
 * 5. 【useMemo 动态生成标题�?
 *    useMemo 缓存计算结果，只�?[slug] 变化时才重新计算�?
 *    标题生成逻辑�?
 *    - 先从内置映射表查找（available-today �?"Available Today"�?
 *    - 再从 occasions 数据中查找（�?anniversary-flowers �?"Anniversary Flowers"�?
...
import occasions from '../common/occasions';
import '../../styles/product.css';
import '../../styles/CollectionPages.css';

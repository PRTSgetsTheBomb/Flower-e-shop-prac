 ...
 * 占位页面 - 用于"建设�?的内容页
 *
 * 核心职责：用一个组件处理多�?尚未开�?的路由，动态生成页面标�?
 *
 * 设计说明�?
 *
 * 1. 【一个组件处理多个路由�?
 *    �?App.js 中，/about�?contact�?delivery-areas�?policies/:policy �?
 *    多个路由都指�?PlaceholderPage。如果为每个页面都写一个空壳组件，
 *    会重复很多相同的代码。共享一个组件是更高效的做法�?
 *
 * 2. 【useLocation 获取当前路径�?
 *    useLocation().pathname 返回当前 URL 的路径部分（�?/policies/refund-policy）�?
 *    通过解析路径动态生成页面标题，让用户感觉每个页面都�?专属"标题�?
 *
 * 3. 【路径解析逻辑�?
 *    - 单段路径�?/about �?segments = ["about"] �?key = "about" �?title = "About Us"
 *    - 多段路径�?/policies/refund-policy �?segments = ["policies", "refund-policy"]
 *      �?优先检查第二个段是否有映射 �?key = "refund-policy" �?title = "Return and Refund Policy"
 *    - 如果映射表中找不到，自动将连字符替换为空格，首字母大�?
 *      （如 "delivery-areas" �?"Delivery Areas"�?
 *
 * 4. 【行内样式的使用场景�?
 *    这里的样式很简单（只有 padding、textAlign、minHeight 等几个属性）�?
 *    没有必要单独写一�?CSS 文件。行内样式适合这种"一次�?的简单布局�?
 *    而独立的 CSS 文件更适合需要复用的复杂样式�?
 *
 * 5. �?建设�?页面的价值�?
 *    在项目初期先用占位页占住路由，让用户知道"这个页面将来有内�?�?
 *    而不是直�?404 或空白页。后续开发对应页面时，只需创建新组�?
 *    并替�?App.js 中的 Route 指向即可�?
 */
...
function PlaceholderPage() {
  ...
  // �?/policies/refund-policy 拆成 ["policies", "refund-policy"]
  const segments = pathname.replace(/^\//, '').split('/');
  // 如果�?/policies/xxx 这类二级路径，取第二段作�?key
